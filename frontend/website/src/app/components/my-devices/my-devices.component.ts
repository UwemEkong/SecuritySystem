...
export class MyDevicesComponent implements OnInit {
  ...
  createDevice(deviceData:any) {
    this.deviceService.createDevice({userid:this.authService.loggedInUser.id, macaddress: deviceData.value.macaddress, name: deviceData.value.name, location: this.locationService.formattedLocation, active:false, defaultdevice:false, motionactive:true}).subscribe((data)=>{
      this.deviceService.getAllUserDevices()
    })
  }
